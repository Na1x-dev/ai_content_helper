...
export default function Pricing() {
  ...
  const handleBuyPlan = async (planCode) => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        window.location.reload(); // Обновляем страницу для синхронизации лимитов в шапке
      }
    } catch (err) {
      const errorMessage = err.response?.data?.message || 'Ошибка при изменении тарифного плана';
      alert(errorMessage);
    } finally {
      setLoadingPlan(null);
    }
  ...
  const currentCard = cards.find(c => c.code === currentPlanCode) || { weight: 0 };
    ...
    <div className="max-w-7xl mx-auto animate-fade-in space-y-8 py-4 px-2">
      <div className="text-center space-y-2">
        ...
        <p className="text-slate-400 text-sm max-w-md mx-auto">
          Вы можете свободно переключаться между тарифами в любой момент в зависимости от ваших задач.
        </p>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="space-y-4 flex-grow flex flex-col justify-between">
                <div>
                  <h3 className={`text-base font-bold text-slate-200 ${card.code !== 'free' && card.code !== 'standard' ? 'text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-indigo-400' : ''}`}>
                    {card.title}
                  </h3>
                ...
                <div className="text-xl font-black text-slate-100 my-2">
                  {card.price} <span className="text-xs font-medium text-slate-500">{card.period}</span>
                </div>
