...
export default function History() {
  ...
  return (
    <div className="space-y-4 max-w-4xl mx-auto animate-fade-in px-2">
      <h2 className="text-xl font-bold tracking-tight text-slate-200 text-left">История публикаций</h2>
      <div className="grid grid-cols-1 gap-4">
        {posts.map((post) => (
          <div key={post.id} className="bg-slate-900/60 backdrop-blur-xl border border-slate-800/80 rounded-2xl p-5 flex flex-col gap-4 relative hover:border-slate-700 transition-colors">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-800/50 pb-3">
              ...
              <div className="flex items-center gap-1.5 text-xs text-slate-500 font-medium flex-shrink-0">
                <Calendar size={13} /> 
                <span>{new Date(post.created_at).toLocaleString('ru-RU', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
              </div>
            </div>
            ...
            <div className="space-y-2.5 text-left">
              <div className="text-xs font-semibold text-slate-400 flex items-start gap-1.5 leading-relaxed">
                <MessageSquare size={13} className="mt-0.5 flex-shrink-0 text-slate-500" /> 
                <span>
                  Запрос: <span className="font-normal text-slate-300 italic">"{cleanPrompt(post.prompt)}"</span>
                </span>
              </div>
              ...
              <div className="relative group">
                ...
                {post.status === 'completed' && post.text && (
                  ...
                  >
                    {copiedId === post.id ? <Check size={13} className="text-emerald-400" /> : <Clipboard size={13} />}
                  </button>
                )}
              </div>
            </div>
